# Set up Engaz with a coding agent

Copy one of the prompts below into a coding agent.

## Published images (no checkout)

This route is pending publication and verification of public Engaz images. Use the source-checkout route below for now.

```text
Set up Engaz from published GHCR images and leave the web UI running.
...
1. Which directory should contain the Engaz folder (or use the current directory)?
2. How should models be connected?
   - Add a deployment-wide `OPENROUTER_API_KEY` to `.env`.
   - Connect during Engaz onboarding with a provider API key or with ChatGPT Plus/Pro, GitHub Copilot, or SuperGrok / X Premium.
...
2. Download and inspect this installer (do not clone the repository):
   https://raw.githubusercontent.com/shadynafie/engaz/main/infra/compose/install-images.sh
   If that host is unreachable, use a mirror URL (e.g. set `ENGAZ_INSTALLER_URL` to
   `https://example.com/mirror/engaz/infra/compose/install-images.sh` and curl that instead).
3. Run `bash install-images.sh --prepare-only`. It downloads the Compose and environment example
   ...
   Optional: set `ENGAZ_DOWNLOAD_BASE` to a generic mirror of `infra/compose`, and/or use
   `--local` / `ENGAZ_DOWNLOAD_SKIP_EXISTING=1` when Compose files are already present locally.
...
```text
Set up Engaz locally and leave it running in a usable state.

Repository: https://github.com/shadynafie/engaz.git
...
1. Should you clone into the current directory, or what parent directory should contain `engaz`? If you are already inside a Engaz checkout, offer to use it without recloning.
2. How should models be connected?
   - Add a deployment-wide `OPENROUTER_API_KEY` to `.env`.
   - Connect during Engaz onboarding with a provider API key or with ChatGPT Plus/Pro, GitHub Copilot, or SuperGrok / X Premium.
   - Defer model setup and verify infrastructure only. Make clear that bots cannot answer until a model is connected.
...
- If I requested Electron, leave the web stack running and then launch `pnpm --filter @engaz/desktop dev`. Verify the shell loads the same app. Let me make the Docker-versus-This-Mac choice because This Mac grants bots access under my OS account.
