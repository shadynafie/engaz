...
import { ServerUpdateRunSchema } from "@engaz/contracts";
import {
...
export interface UpdaterProxyConfig {
  ...
  /** Current `ENGAZ_IMAGE_TAG` when known; selects compose pull vs rebuild commands. */
...
export async function readServerUpdateStatus(
  ...
  const imageTagHint = config.imageTag?.trim() || process.env.ENGAZ_IMAGE_TAG?.trim() || null;
