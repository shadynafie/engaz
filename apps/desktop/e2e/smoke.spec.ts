...
import type { EngazDesktop } from "@engaz/contracts";
import { _electron as electron, expect, test } from "@playwright/test";

const fixture = `<!doctype html>
<html lang="en">
  <head><meta charset="utf-8"><title>Engaz desktop smoke</title></head>
...
test("launches with a narrow preload bridge and an isolated renderer", async () => {
  ...
  try {
    ...
    await expect(page).toHaveTitle("Engaz desktop smoke");

    const renderer = await page.evaluate(async () => {
      const desktop = (window as typeof window & { engazDesktop?: EngazDesktop }).engazDesktop;
