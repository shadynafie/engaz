...
import type { EngazDesktop, EngazSetup } from "@engaz/contracts";
...
describe("desktop preload bridge", () => {
  it("exposes the scoped desktop bridges", async () => {
    ...
    const [globalName, bridge] = exposeInMainWorld.mock.calls[0] as [string, EngazDesktop];
    expect(globalName).toBe("engazDesktop");
  ...
  it("forwards captured codes without leaking the IPC event to the renderer", () => {
    ...
    const [, bridge] = exposeInMainWorld.mock.calls[0] as [string, EngazDesktop];
...
describe("setup preload bridge", () => {
  it("exposes only the first-run setup operations", async () => {
    ...
    expect(exposeInMainWorld).toHaveBeenCalledTimes(1);
    const [globalName, bridge] = exposeInMainWorld.mock.calls[0] as [string, EngazSetup];
    expect(globalName).toBe("engazSetup");
    expect(bridge.platform).toBe("linux");
    expect(Object.keys(bridge).sort()).toEqual([
      "openLink",
      "platform",
