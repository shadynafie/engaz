...
describe("server address normalization", () => {
  ...
  it("keeps an explicit secure scheme and port but stores only the origin", () => {
    expect(normalizeServerUrl("https://engaz.example.com")).toBe("https://engaz.example.com");
    expect(normalizeServerUrl("https://engaz.example.com:8443/team")).toBe(
      "https://engaz.example.com:8443",
    );
  });
  ...
  it.each(["", "   ", "not a url", "ftp://example.com", "file:///etc/passwd", "http://"])(
    "rejects an address that cannot reach a Engaz server (%s)",
    (value) => {
  ...
  it("rejects embedded credentials rather than writing them to disk", () => {
    expect(normalizeServerUrl("https://user:secret@engaz.example.com")).toBeNull();
  });
});
...
describe("saved setup", () => {
  it("round-trips through the on-disk format", () => {
    const setup = { mode: "existing", serverUrl: "https://engaz.example.com" } as const;
...
describe("startup target", () => {
  const saved = { mode: "existing", serverUrl: "https://engaz.example.com" } as const;
  ...
  it("opens the saved instance on later launches", () => {
    expect(resolveStartupTarget({ saved })).toEqual({
      kind: "app",
      url: "https://engaz.example.com",
      source: "saved",
    });
  });

  it("lets ENGAZ_WEB_URL point the shell anywhere without touching saved setup", () => {
  ...
  it("ignores an empty ENGAZ_WEB_URL", () => {
...
describe("bundled renderer eligibility", () => {
  it("stands in for http(s) origins only", () => {
    ...
    expect(servesBundledRenderer("https://engaz.example.com")).toBe(true);
...
describe("remote-content isolation", () => {
  it("uses a stable, opaque session partition per server origin", () => {
    ...
    expect(first).toMatch(/^persist:engaz-[a-f0-9]{24}$/);
...
describe("Engaz health response", () => {
  it("requires the public RPC health contract", () => {
    expect(isEngazHealth({ json: { ok: true, version: "0.1.0" } })).toBe(true);
    expect(isEngazHealth({ json: { ok: true } })).toBe(false);
    expect(isEngazHealth({ ok: true, version: "0.1.0" })).toBe(false);
  });
});
