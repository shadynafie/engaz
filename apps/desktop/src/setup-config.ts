...
import type { DesktopSetup, DesktopStackProbeResponse } from "@engaz/contracts";
...
export function normalizeServerUrl(input: string): string | null {
  ...
  // Engaz serves its renderer, RPC, and auth routes from one origin. Keeping a
 ...
 * `ENGAZ_WEB_URL` still wins over saved configuration so test and performance
...
/** Each Engaz origin gets its own persistent cookie and storage partition. */
...
export function isEngazHealth(value: unknown): boolean {
 ...
 * metadata endpoints. Cleartext HTTP to them is never a legitimate Engaz
