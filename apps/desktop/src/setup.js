(() => {
  const bridge = window.engazSetup;
  ...
  const PHASE_LABELS = {
    ...
    pulling: "Downloading Engaz…",
    starting: "Starting Engaz…",
    "waiting-healthy": "Almost ready…",
    ready: "Engaz is ready.",
  };
  ...
  async function check() {
    ...
    try {
      ...
      if (result.ok) {
        ...
        setStatus(`Engaz answered at ${result.url}.`, "ok");
      } else {
  ...
  async function init() {
    ...
    try {
      ...
      setStatus("Setup could not start. Quit Engaz and try again.", "error");
      setBusy(true);
    }
  }
