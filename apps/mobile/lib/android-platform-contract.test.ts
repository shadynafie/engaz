...
describe("Android mobile platform contract", () => {
  ...
  it("requests live-update promotion and exposes its Android settings", () => {
    const nativeRoot = resolve(
      mobileRoot,
      "modules/engaz-notifications/android/src/main/java/com/engaz/notifications",
    );
    const service = readFileSync(resolve(nativeRoot, "EngazNotificationService.kt"), "utf8");
    const module = readFileSync(resolve(nativeRoot, "EngazNotificationsModule.kt"), "utf8");
    ...
    expect(module).toContain("EngazNotificationService.clearSession(context)");
    ...
    expect(service).toContain('connection.setRequestProperty("x-engaz-space-id", spaceId)');
    ...
    expect(service).toContain('"engaz://group-thread?groupId=');
    expect(service).toMatch(/&spaceId=\$\{Uri\.encode\(run\.spaceId\)\}/);
    expect(service).toContain('putString("engaz.spaceId", run.spaceId)');
  ...
  it("shows live updates only for working runs and ties the pill to a real bot", () => {
    const service = readFileSync(
      resolve(
        mobileRoot,
        "modules/engaz-notifications/android/src/main/java/com/engaz/notifications/EngazNotificationService.kt",
      ),
      "utf8",
    );
    expect(service).toContain(
    ...
    expect(service).toContain('putString("engaz.botId", run.botId)');
