...
describe("normalizeApiBase", () => {
  it("trims, adds https, and keeps only the origin", () => {
    ...
    expect(normalizeApiBase("https://engaz.example.com:8443/api/")).toEqual({
      ok: true,
...
describe("display and warnings", () => {
  ...
  it("treats the compile-time default as not custom", () => {
    ...
    expect(usesCustomApiBase("https://engaz.example.com")).toBe(true);
  });
});

describe("probeApiBase", () => {
  it("accepts a Engaz /rpc/health response", async () => {
  ...
  it("rejects a host that is up but is not Engaz", async () => {
    const fetchImpl = vi.fn(
