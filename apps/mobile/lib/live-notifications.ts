...
const nativeNotifications =
  Platform.OS === "android" ? requireNativeModule<NativeNotifications>("EngazNotifications") : null;
...
export function notificationTargetsThread(
  ...
  const dataThreadId = data.threadId ?? data["engaz.threadId"];
  if (target.threadId && dataThreadId) return dataThreadId === target.threadId;
  return Boolean(
    target.botId && (data.botId === target.botId || data["engaz.botId"] === target.botId),
  );
}
