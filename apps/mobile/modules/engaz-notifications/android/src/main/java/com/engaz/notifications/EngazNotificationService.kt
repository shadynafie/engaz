...
class EngazNotificationService : Service() {
  ...
  private fun post(run: RunRecord, copy: NotificationCopy) {
    ...
    val notification = builder(copy.channel)
      .setSmallIcon(R.drawable.ic_engaz_notification)
      .setContentTitle(copy.title)
      .setContentText(copy.body)
      .setStyle(Notification.BigTextStyle().bigText(copy.body))
      .setContentIntent(openApp(run))
      .addExtras(Bundle().apply {
        putString("engaz.spaceId", run.spaceId)
        putString("engaz.botId", run.botId)
        putString("engaz.threadId", run.threadId)
      })
      .setAutoCancel(true)
  ...
  private fun liveStatusIcon(run: RunRecord, avatarStyle: String): Icon {
    if (avatarStyle != "organic") {
      return Icon.createWithResource(this, R.drawable.ic_engaz_notification)
    }
  ...
  private fun openApp(run: RunRecord? = null): PendingIntent {
    val intent = if (run == null) {
      packageManager.getLaunchIntentForPackage(packageName) ?: Intent(Intent.ACTION_VIEW, Uri.parse("engaz://"))
    } else {
      val destination = if (run.groupId != null) {
        "engaz://group-thread?groupId=${Uri.encode(run.groupId)}&name=${Uri.encode(run.groupName.orEmpty())}&spaceId=${Uri.encode(run.spaceId)}"
      } else {
        "engaz://thread?botId=${Uri.encode(run.botId)}&name=${Uri.encode(run.botName)}&spaceId=${Uri.encode(run.spaceId)}"
      }
      Intent(
        Intent.ACTION_VIEW,
  ...
  companion object {
    ...
    private const val ACTION_THREAD_CHANGED = "com.engaz.notifications.THREAD_CHANGED"
    private const val PROMOTED_ONGOING_EXTRA = "android.requestPromotedOngoing"
    private const val POLL_INTERVAL_MS = 8_000L
    private const val STATE_PREFERENCES = "com.engaz.notification_state"
    ...
    fun start(context: Context) {
      val intent = Intent(context, EngazNotificationService::class.java)
      context.startService(intent)
    }

    fun stop(context: Context) {
      context.stopService(Intent(context, EngazNotificationService::class.java))
    }
    ...
    fun setOpenThread(context: Context, botId: String?, threadId: String?) {
      ...
      context.startService(
        Intent(context, EngazNotificationService::class.java).setAction(ACTION_THREAD_CHANGED),
      )
    }
...
private object Channels {
  const val LIVE = "engaz_live"
  const val MESSAGES = "engaz_messages"
  const val SCHEDULED = "engaz_scheduled"
  const val ATTENTION = "engaz_attention"
}
...
private fun rpc(
  ...
  return try {
    ...
    connection.setRequestProperty("Origin", "engaz://")
    connection.setRequestProperty("Authorization", "Bearer $token")
    if (spaceId.isNotBlank()) {
      connection.setRequestProperty("x-engaz-space-id", spaceId)
    }
