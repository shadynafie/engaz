...
import type { DesktopStackProbeResponse } from "@engaz/contracts";
import {
  safeScreenProxyResponseHeaders,
  stripSensitiveHandshakeHeaders,
} from "@engaz/core/node/screen-proxy-response";
...
export default defineConfig(({ mode }) => {
  ...
  const performanceAssetDelayMs = Number(process.env.ENGAZ_PERFORMANCE_ASSET_DELAY_MS ?? 0);
  const desktopStackToken =
    process.env.ENGAZ_DESKTOP_STACK_TOKEN ?? rootEnv.ENGAZ_DESKTOP_STACK_TOKEN ?? "";
  const imageTag = process.env.ENGAZ_IMAGE_TAG ?? rootEnv.ENGAZ_IMAGE_TAG ?? "edge";
  return {
    plugins: [
      react(),
      ...
      tailwindcss(),
      {
        name: "engaz-desktop-stack-probe",
      ...
      {
        name: "engaz-performance-asset-delay",
        configurePreviewServer(server) {
