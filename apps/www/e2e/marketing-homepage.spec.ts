...
test("homepage tells one product story and keeps setup clear", async ({ page }, testInfo) => {
  await page.goto("/");
  ...
  await expect(page.locator("#selfhost h2")).toHaveText("Your installation. Your choices.");
  ...
  await expect(page.locator("#how-it-works, #why-engaz, [data-install-command]")).toHaveCount(0);
