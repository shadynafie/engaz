...
describe.each(caddyfiles)("%s deployment retry", (filename) => {
  ...
  it("applies the retry policy to every API and web upstream", () => {
    expect(config.match(/reverse_proxy api:7792 \{\s*import deploy_retry\s*\}/g)).toHaveLength(3);
    expect(config.match(/reverse_proxy web:7791 \{\s*import deploy_retry\s*\}/g)).toHaveLength(1);
  });
});
