...
const app = createUpdaterApp(
  resolveUpdaterConfig({
    ENGAZ_DEPLOY_DIR: "/engaz-updater-tests-no-such-directory",
...
async function deployment(env = "ENGAZ_IMAGE_TAG=v1.0.0\nENGAZ_IMAGE_TAG_PREVIOUS=v0.9.0\n") {
  const deployDir = await mkdtemp(path.join(os.tmpdir(), "engaz-updater-test-"));
  temporaryDirectories.push(deployDir);
  await writeFile(path.join(deployDir, ".env"), env);
  return {
    deployDir,
    config: resolveUpdaterConfig({ ENGAZ_DEPLOY_DIR: deployDir, ENGAZ_UPDATER_TOKEN: token }),
  };
}
...
describe("updater HTTP surface", () => {
  ...
  it("re-validates the branch at its own boundary", async () => {
    const response = await app.request("/apply", {
      method: "POST",
      headers: authorized,
      body: JSON.stringify({ repoUrl: "https://github.com/shadynafie/engaz", branch: "--exec=id" }),
    });
...
describe("updater orchestration", () => {
  ...
  it("restores the prior image after a recreate fails health checks", async () => {
    ...
    expect(composeUp[0]?.env?.ENGAZ_IMAGE_TAG).toBe(`sha-${targetCommit}`);
    expect(composeUp[1]?.env?.ENGAZ_IMAGE_TAG).toBe("v1.0.0");
  ...
  it("resets a fork checkout when recreate fails after the fast-forward", async () => {
    const fixture = await deployment("ENGAZ_IMAGE_TAG=local\n");
    await mkdir(path.join(fixture.deployDir, ".git"));
    const calls: string[][] = [];
    let upCalls = 0;
    const run: UpdaterCommandRunner = async (command, args) => {
  ...
  it("refuses to replace a symlinked deployment environment", async () => {
    ...
    await writeFile(target, "ENGAZ_IMAGE_TAG=v1.0.0\n");
    await rm(envFile);
    ...
    expect(await readFile(target, "utf8")).toBe("ENGAZ_IMAGE_TAG=v1.0.0\n");
  });
...
describe("child process environment", () => {
  it("declares the deployment directory safe so git works on a non-root checkout", () => {
    ...
    const env = commandEnvironment({ ENGAZ_DEPLOY_DIR: "/srv/engaz" });
    expect(env.GIT_CONFIG_COUNT).toBe("1");
    expect(env.GIT_CONFIG_KEY_0).toBe("safe.directory");
    expect(env.GIT_CONFIG_VALUE_0).toBe("/srv/engaz");
  });

  it("keeps the exemption scoped to the deployment directory, whoever calls it", () => {
    const env = commandEnvironment(
      { ENGAZ_DEPLOY_DIR: "/opt/engaz" },
      { GIT_CONFIG_VALUE_0: "*", GIT_CONFIG_COUNT: "9" },
    );
    expect(env.GIT_CONFIG_VALUE_0).toBe("/opt/engaz");
    expect(env.GIT_CONFIG_COUNT).toBe("1");
  });
