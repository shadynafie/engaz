...
import { type Logger, SERVICE_NAMES } from "@engaz/logging";
...
export function commandEnvironment(
  ...
  const deployDir = source.ENGAZ_DEPLOY_DIR?.trim();
  const gitOwnership =
...
export function createUpdaterApp(
  ...
  async function writeEnvAssignments(assignments: Record<string, string>) {
    ...
    const temporary = `${config.envFile}.engaz-update-${randomUUID()}`;
  ...
  async function apply(request: { repoUrl: string; branch: string; official: boolean }) {
    ...
    if (decision.strategy === "build") {
      if (!checkout.present) {
        throw new UpdateRefused(
          "Building a fork needs the deployment's git checkout, and ENGAZ_DEPLOY_DIR has no .git directory. Clone the fork to the deployment directory, or switch back to the official repository to use published images.",
        );
      }
