import type { AdapterContext, ConnectorEvent, ConnectorTool } from "@engaz/adapter-kit";
...
describe("composio tool mapping", () => {
  ...
  it("uses catalog-canonical toolkit slugs without preloading every tool", async () => {
    ...
    expect(
      ...
      { userId: "__engaz_catalog__", toolkits: undefined, sessionPreset: undefined },
  ...
  it("only fetches live Composio slugs when a Engaz row is still pending or errored", () => {
  ...
  it("plans DB sync when Composio is connected but Engaz is still pending", () => {
    expect(
      planLiveConnectionSync(
        [
