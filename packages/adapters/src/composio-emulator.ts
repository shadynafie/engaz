...
import {
  DEFAULT_ENGAZ_EMULATED_RELEASES,
    ...
    messages: [
      {
        ...
        subject: "Welcome to Engaz",
        sender: "hello@engaz.test",
        to: "me@example.test",
  ...
  return [
    {
      ...
      inputSchema: {
        type: "object",
        properties: {
          owner: { type: "string", description: "Repository owner, e.g. shadynafie" },
          repo: { type: "string", description: "Repository name, e.g. engaz" },
        },
        required: ["owner", "repo"],
      },
    },
...
export class ComposioEmulator implements ComposioProvider {
  ...
  private githubReleases: EmulatedGithubRelease[] = [...DEFAULT_ENGAZ_EMULATED_RELEASES];
  ...
  private executeGithub(tool: string, args: Record<string, unknown>): Record<string, unknown> {
    const owner = String(args.owner ?? args.owner_name ?? "shadynafie");
    const repo = String(args.repo ?? args.repository ?? "engaz");
