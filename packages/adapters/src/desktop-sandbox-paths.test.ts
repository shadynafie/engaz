...
describe("desktop sandbox path rules", () => {
  it("compares Windows roots case-insensitively without accepting siblings or other drives", () => {
    const roots = ["C:\\Users\\Owner\\Engaz\\bot"];

    expect(isAllowedDesktopPath("c:\\users\\owner\\engaz\\BOT\\notes.txt", roots, path.win32)).toBe(
      true,
    );
    expect(
      isAllowedDesktopPath("C:\\Users\\Owner\\Engaz\\bot-other\\notes.txt", roots, path.win32),
    ).toBe(false);
  ...
  it("handles explicitly allowed UNC roots without accepting sibling shares", () => {
    const roots = ["\\\\server\\share\\engaz\\bot"];

    expect(
      isAllowedDesktopPath("\\\\SERVER\\share\\engaz\\BOT\\notes.txt", roots, path.win32),
    ).toBe(true);
    expect(
      isAllowedDesktopPath("\\\\server\\share-other\\engaz\\bot\\notes.txt", roots, path.win32),
    ).toBe(false);
  });
