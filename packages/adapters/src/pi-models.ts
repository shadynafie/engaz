...
import type { ModelOAuthSignInMode, ThinkingLevel } from "@engaz/contracts";
...
function catalogBilling(
  ...
  if (providerId === LOCAL_PROVIDER_ID) {
    return "Runs on infrastructure configured by the deployment owner. No model charges from Engaz.";
  }
  if (providerId === OPENAI_COMPATIBLE_PROVIDER_ID) {
    return "Runs on a URL you control. Engaz does not pay for model usage.";
  }
  if (opts.oauth && !opts.apiKey) {
    return `${name} subscription login is not in the Engaz UI yet. Skip if this deployment already has credentials.`;
  }
  if (opts.apiKey) {
    return `Uses your ${name} API key. Engaz does not pay for model usage.`;
  }
  return `Uses your ${name} key. Engaz does not pay for model usage.`;
}
