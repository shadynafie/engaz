...
export function serenityBotEntity(botId: string, brainLabel = ""): string {
  ...
  return label ? `engaz-bot/${label}/${botId}` : `engaz-bot/${botId}`;
}

export function serenitySpaceEntity(spaceId: string, brainLabel = ""): string {
  const label = sanitizeSerenityBrainLabel(brainLabel);
  return label ? `engaz-space/${label}/${spaceId}` : `engaz-space/${spaceId}`;
}
...
export class SerenityMemoryProvider implements SemanticMemoryProvider {
  ...
  async recall(
    ...
    // History compaction stays in Engaz; Serenity is the durable brain only.
  ...
  async save(
    ...
    const provenance = `engaz space:${context.spaceId} bot:${request.botId}`;
    const results = await Promise.all(
      entities.map((entity) =>
