...
const temporaryRoot = await mkdtemp(path.join(os.tmpdir(), "engaz-desktop-performance-"));
...
function performanceEnvironment(databaseUrl: string): NodeJS.ProcessEnv {
  return {
    ...
    E2B_API_KEY: "",
    DAYTONA_API_KEY: "",
    BETTER_AUTH_SECRET: "engaz-benchmark-auth-secret-over-32-characters",
    ENCRYPTION_KEY: "engaz-benchmark-encryption-key-over-32-characters",
    SANDBOX_SUPERVISOR_TOKEN: "engaz-benchmark-supervisor-token-over-32-characters",
    SCREEN_PROXY_SECRET: "engaz-benchmark-screen-proxy-secret-over-32-characters",
    ...
    ENGAZ_HOST: "127.0.0.1",
    ENGAZ_WEB_URL: webOrigin,
    ENGAZ_DISABLE_BUNDLED_RENDERER: remoteRenderer ? "1" : "0",
    ENGAZ_DISABLE_WARM_WINDOW: disableWarmWindow ? "1" : "0",
...
function buildProductionArtifacts(env: NodeJS.ProcessEnv) {
  run("pnpm", ["--filter", "@engaz/desktop", "pack:dir"], env);
}

function migrateDatabase(env: NodeJS.ProcessEnv) {
  run("pnpm", ["--filter", "@engaz/db", "generate"], env);
  run("pnpm", ["--filter", "@engaz/db", "exec", "prisma", "migrate", "deploy"], env);
}
...
async function packagedExecutable() {
  ...
  const candidates = process.platform === "darwin" ? await findNamed(out, "Engaz.app") : [];
  if (process.platform === "darwin" && candidates[0]) {
    return path.join(candidates[0], "Contents/MacOS/Engaz");
  }
...
async function prepareAuthenticatedProfile(benchmark: BenchmarkContext, profile: string) {
  ...
  try {
    ...
    await page.getByPlaceholder("Your email address").fill(`benchmark-${stamp}@engaz.test`);
...
async function launchDesktop(
  ...
  const app = await electron.launch({
    ...
    env: {
      ...benchmark.env,
      ENGAZ_PERFORMANCE_USER_DATA: profile,
      ENGAZ_PERFORMANCE_CLEAR_CACHE: clearCache ? "1" : "0",
    },
  });
...
async function measureInteractions(app: ElectronApplication, page: Page) {
  ...
  await page.evaluate(() => {
    ...
    (window as typeof window & { __engazKeyPaintSamples?: number[] }).__engazKeyPaintSamples =
      samples;
...
async function measureBundles() {
  ...
  const applications = await findNamed(path.join(desktopRoot, "out"), "Engaz.app");
...
function renderMarkdown(report: PerformanceReport) {
  ...
  return `# Engaz desktop performance — ${report.label}
