...
import type { ComputerRef, SandboxProvider } from "@engaz/adapter-kit";
...
describeLive("real model and sandbox computer journey", () => {
  ...
  beforeAll(async () => {
    ...
    dataDir = mkdtempSync(path.join(tmpdir(), "engaz-computer-e2e-"));
...
PAGE = b'''<!doctype html><meta charset="utf-8"><title>Engaz visual test</title>
