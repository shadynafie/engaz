...
import { createThreadEvents, createThreadMessage, loadRunHistoryMessages } from "@engaz/db";
...
describeIntegration("run executor lifecycle", () => {
  ...
  const dataDir = mkdtempSync(path.join(tmpdir(), "engaz-executor-lifecycle-"));
  ...
  it("records an uncertain result without replaying an interrupted external effect", async () => {
    ...
    const args = { collection: "notes", title: "Engaz result", body: prompt };
  ...
  it("applies the same no-parallel-run rule to the targeted group member", async () => {
    const cookie = await signup(
      `executor-group-steering-${stamp}@engaz.test`,
