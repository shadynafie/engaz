...
import type { SearchHit, ThreadSnapshot } from "@engaz/contracts";
...
describeSearch("workspace search", () => {
  ...
  const dataDir = mkdtempSync(path.join(tmpdir(), "engaz-search-"));
  ...
  it("finds bots, messages, files, links, and routines within the workspace", async () => {
    const cookie = await signup(app, `search-${stamp}@engaz.test`, "Search User");
  ...
  it("returns no hits for another workspace", async () => {
    const ownerCookie = await signup(app, `search-owner-${stamp}@engaz.test`, "Owner");
    ...
    const intruderCookie = await signup(app, `search-intruder-${stamp}@engaz.test`, "Intruder");
  ...
  it("finds group conversations, messages, and files", async () => {
    const cookie = await signup(app, `search-group-${stamp}@engaz.test`, "Group Search User");
