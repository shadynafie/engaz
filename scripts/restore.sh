...
until "${compose[@]}" exec -T postgres pg_isready -U engaz >/dev/null 2>&1; do
  sleep 1
done
...
"${compose[@]}" exec -T postgres psql -X -U engaz -d engaz \
  --set=ON_ERROR_STOP=on --single-transaction --file=- < "$SRC/engaz.sql"
